...
export default function HotelListing() {
    ...
    const getHotels = async () => {
        ...
        try {
            ...
            const mappedData: IHotel[] = res.data.map((hotel: { _id: string; name: string }) => {
                return {
    ...
    if (!data.length) return <EmptyState message="No hotels found for your search." />;

    return (
        <div className="bg-[#f7f8f8] min-h-screen">
            <Filter>
                {currentHotels.map((hotel: IHotel) => (
                    <div key={hotel._id} className="bg-white rounded-2xl p-6 m-4 shadow-md">
                        ...
                        <div className="flex gap-4 items-center mt-4">
                            <Button
                                ...
                                aria-label={isFavourite(hotel._id) ? "Remove from favourites" : "Add to favourites"}
                            >
                                <Heart
